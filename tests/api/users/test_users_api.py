...
class TestUsersApi(TestUsersBase):
    # To get token please register here -> https://gorest.co.in/user/login.html and put it into the `dev.ini`
    ...
    def test_crud_user(self):
        ...
        with step('Check user appeared in the list'):
            users = self.users_api.get_all_users(name=generated_body['dict']['name'])
        ...
        with step('Check user missed in the list'):
            users = self.users_api.get_all_users(first_name=generated_body['dict']['name'])
            assert created_user['id'] not in [_['id'] for _ in users], 'Deleted user still present in the total list'
